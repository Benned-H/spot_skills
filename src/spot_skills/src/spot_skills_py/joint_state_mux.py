...
from spot_skills.srv import NameService, NameServiceRequest, NameServiceResponse
...
class JointStateMux:
    ...
    def __init__(self) -> None:
        ...
        self.service = rospy.Service("set_joint_mode", NameService, self.handle_set_mode)
    ...
    def find_default_mode(cls) -> str:
        ...
        """
        give_tamp_control = get_ros_param("~tamp_controls_robot", bool)
    ...
    def publish_state(self) -> None:
        ...
        if curr_config is None:
            raise KeyError(f"Current joint state mode '{self._mode}' is invalid!")
    ...
    def joint_state_callback(self, msg: JointState, relevant_mode: str) -> None:
        ...
        if relevant_mode not in self.latest_configs:
            raise ValueError(f"Unrecognized joint state mode: '{relevant_mode}'.")
    ...
    def handle_set_mode(self, request: NameServiceRequest) -> NameServiceResponse:
        """Handle a request to set the mode of the joint state multiplexer.

        :param request: ROS message specifying the name of the new joint state mode
        :return: Response message indicating whether or not the request succeeded
        """
        if request.name not in self.valid_modes:
            message = f"Cannot set joint state mode to unrecognized mode: '{request.name}'."
            return NameServiceResponse(success=False, message=message)

        self._mode = request.name
        message = f"Switched joint state mode to '{self._mode}'."
        rospy.loginfo(message)

        return NameServiceResponse(success=(self._mode == request.name), message=message)
